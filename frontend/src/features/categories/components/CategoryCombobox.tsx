...
export function CategoryCombobox({
  ...
  const filteredCategories = React.useMemo(() => {
    ...
    if (excludedCategoryIds.length > 0) {
      categories = categories.filter(
        (cat) => cat.id && !excludedCategoryIds.includes(cat.id)
      );
    }

    return categories;
  }, [allCategories, filterByType, excludedCategoryIds]);
  ...
  const handleCreateCategory = (values: any) => {
    createCategoryMutation.mutate(
      { data: { ...values, categoryType: filterByType || "EXPENSE" } },
      {
  ...
  return (
    <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            Create a New {filterByType === "INCOME" ? "Income" : "Expense"}{" "}
            Category
          </DialogTitle>
        </DialogHeader>
        <CategoryForm
